...
// tuner_test.go tests whether the evaluation is symmetrical, and displays what
// values the evaluation finds to make sure they're reasonable.
...
func TestEvaluation(t *testing.T) {
	for _, fen := range TestFENs {
		...
		stmEval := EvaluatePos(&pos)
		pos.SideToMove ^= 1
		oppEval := EvaluatePos(&pos)
		pos.SideToMove ^= 1

		if stmEval != -oppEval {
			t.Errorf(
				"For position %s [%s], got %d for side to move evaluation, but %d for opponet",
				pos.String(), fen, stmEval, oppEval,
			)
		} else {
			t.Logf(
				"Pass for position %s: side to move evaluation = %d",
				fen, stmEval,
			)
		}
	}
