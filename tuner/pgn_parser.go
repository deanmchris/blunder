...
func parsePGNs(filename string, minimumElo int) (pgns []PGN, skipped int) {
	...
	for _, chunk := range chunks {
		...
		if resultTag == "" {
			log.Println("invalid pgn skipped: no results")
			skipped++
			continue
		} else {
		...
		if whiteEloTag == "" {
			log.Println("invalid pgn skipped: no elo for white")
			skipped++
			continue
		} else {
			eloStr := strings.Fields(whiteEloTag)[1]
			eloStr = strings.TrimPrefix(eloStr, "\"")
			eloStr = strings.TrimSuffix(eloStr, "\"]")

			elo, err := strconv.Atoi(eloStr)
			if err != nil {
				log.Println("invalid pgn skipped: white elo in unexpected format")
				skipped++
				continue
			}

			if elo < minimumElo {
				log.Println("unwanted pgn skipped: white elo too low")
				skipped++
				continue
			}
		}
		...
		if blackEloTag == "" {
			log.Println("invalid pgn skipped: no elo for black")
			skipped++
			continue
		} else {
			eloStr := strings.Fields(blackEloTag)[1]
			eloStr = strings.TrimPrefix(eloStr, "\"")
			eloStr = strings.TrimSuffix(eloStr, "\"]")

			elo, err := strconv.Atoi(eloStr)
			if err != nil {
				log.Println("invalid pgn skipped: black elo in unexpected format")
				skipped++
				continue
			}

			if elo < minimumElo {
				log.Println("unwanted pgn skipped: black elo too low")
				skipped++
				continue
			}
		}
